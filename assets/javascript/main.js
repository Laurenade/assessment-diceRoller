...
rollButton.addEventListener("click", function() {                              // (2) //    
    ...
    let currentRoll = 0                                                        // (4...???) //
        while (currentRoll <= valueEnteredTextbox) {      ///???? not looping with the textbox input amount///
...
//console.log('good')
    ...
    // 4- Add that List Item to the innerHTML of the "All Rolls" (listOfResults) element
        ...
        resultsButton.addEventListener("click", function() {                 // (1) //
                                        //???? let toListNumber = ____
                let number = 0
            while (number < dieRolls.length) {                               // (2) //
                toListNumber = dieRolls[number]                              // (3...???) //
                                        //????  Error:Cannot access'toListNumber'before initialization
